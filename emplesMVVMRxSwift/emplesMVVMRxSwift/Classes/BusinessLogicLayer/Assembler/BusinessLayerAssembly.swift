...
class BusinessLayerAssembly: Assembly {
    ...
    func assemble(container: Container) {
        container.register(EmplesFSJsonReader.self) { _ in EmplesFSJsonReader() }.inObjectScope(.container)
        container.register(DataAreaGatewayProtocol.self) { r in
            DataFilePersistenceGateway(with: r.resolve(EmplesFSJsonReader.self)!) }.inObjectScope(.container)
